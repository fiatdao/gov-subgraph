...
import {Comitium} from "../generated/Comitium/Comitium";
...
export namespace common {
    ...
    export function getOverview(): Overview {
        ...
        if (overview == null) {
            ...
            overview.totalVFDT = constants.BIGINT_ZERO;
            overview.voters = 0
            overview.comitiumUsers = 0;
            overview.proposals = 0;
            overview.totalFDTAirdropClaimed = constants.BIGINT_ZERO;
            overview.totalFDTAirdropRedistributed = constants.BIGINT_ZERO;
    ...
    export function updateHolders(change: i32): void {
        ...
        overview.holders = overview.comitiumUsers + overview._holdersWithNonZeroBalance;
        overview.save();
    }

    export function updateVotingPower(user: Address, contractAddress: Address): void {
        let voter = common.createVoterIfNonExistent(user);

        let oldVotingPower = voter.votingPower;
        let contract = Comitium.bind(contractAddress);
        voter.votingPower = contract.votingPower(user);

        let overview = common.getOverview();
        overview.totalVFDT = overview.totalVFDT.minus(oldVotingPower).plus(voter.votingPower);
    ...
    // @ts-ignore
    export function updateComitiumUsers(change: i32): void {
        let overview = getOverview();
        overview.comitiumUsers += change;
        overview.save();
    }
}
